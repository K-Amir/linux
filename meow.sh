...
# --- Max SYN backlog (increased for 2000 users) ---
if [[ $TOTAL_RAM_MB -le 1200 ]]; then
    MAX_SYN_BACKLOG=32768   # Doubled for high load
elif [[ $TOTAL_RAM_MB -le 2500 ]]; then
    MAX_SYN_BACKLOG=65536   # High capacity
else
    MAX_SYN_BACKLOG=131072  # Maximum capacity
fi
...
########################################
# 4. Load Conntrack Module and Set Buckets
########################################
echo -e "${GREEN}Loading connection tracking module...${NC}"
...
# Set conntrack buckets BEFORE applying sysctl (read-only after module loads)
CONNTRACK_BUCKETS=$((CONNTRACK_MAX / 4))
if [[ -f /sys/module/nf_conntrack/parameters/hashsize ]]; then
    echo $((CONNTRACK_BUCKETS)) > /sys/module/nf_conntrack/parameters/hashsize 2>/dev/null || true
    echo -e "${GREEN}Conntrack buckets set to: ${CONNTRACK_BUCKETS}${NC}"
fi

########################################
# 5. Backup Existing sysctl Configuration
########################################
...
########################################
# 6. Apply RAM-Aware sysctl Settings
########################################
...
# Load profile: Up to 2000 users per node with massive headroom
...
########################################
# 7. Optimize Network Interface Settings
########################################
...
# Enable UDP GRO for Hysteria2 performance
ethtool -K "$DEFAULT_NIC" rx-udp-gro-forwarding on 2>/dev/null || {
    echo -e "${YELLOW}UDP GRO forwarding not supported on this NIC${NC}"
}
...
########################################
# 8. Multi-Core Optimization (RPS/RFS)
########################################
...
########################################
# 9. Set System Limits for Xray/V2Ray
########################################
...
########################################
# 10. Systemd Override for x-ui / Xray / Hysteria
########################################
...
echo -e "${GREEN}    - Capacity: Up to 2000 users per node${NC}"
echo -e "${GREEN}========================================${NC}"
