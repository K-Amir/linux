...
# Configuration
MEMORY_THRESHOLD=85       # Percentage threshold to trigger actions
SWAP_THRESHOLD=90         # Percentage threshold for swap usage
...
if [ "$MEM_USAGE" -gt "$MEMORY_THRESHOLD" ]; then
    ...
    if [ "$MEM_USAGE" -gt 95 ]; then
        # Critical memory pressure - take more aggressive action
        log "CRITICAL: Memory usage above 95%. Taking emergency actions."
        sync
        echo 3 > /proc/sys/vm/drop_caches
        echo 1 > /proc/sys/vm/compact_memory
        
        # If swap usage is also high, consider restarting the service
        if [ "$SWAP_USAGE" -gt "$SWAP_THRESHOLD" ]; then
            log "High swap usage detected. Restarting $V2RAY_SERVICE to free memory..."
            systemctl restart "$V2RAY_SERVICE"
        fi
    elif [ "$MEM_USAGE" -gt 90 ]; then
        # High memory pressure
        log "HIGH: Memory usage above 90%. Clearing page cache and dentries."
        sync
...
# Create a cron job to run the memory manager every 5 minutes
cat > /etc/cron.d/memory-manager << 'EOF'
# Run memory management script every 5 minutes
*/5 * * * * root /usr/local/bin/memory-manager >/dev/null 2>&1
EOF
